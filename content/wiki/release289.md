...
There have been major changes in 2.89 (see below).  From now on, you are more likely to be required to use programme identifiers (PIDs) to download programmes than in the past.  Now is the time to acquaint yourself with this mode of operation by reading [this](/wiki/documentation#recording) and [this](/guides/tv-download-guide/#how-do-i-record-a-programme-with-get-iplayer) (external link).  Then **try it yourself** before asking for help.  It's documented and involves only a few extra keystrokes, so it should not present an obstacle to you.  The other thing you must remember about downloading via PID is that you must include `--type=radio` in your command when downloading radio programmes with `--pid`.  
